...
public class FtpUtil {
     ...
     * FTP服务器hostname
     */
    private static String HOST = "139.224.211.24";
    /**
     * FTP服务器端口
     */
     ...
     * FTP登录密码
     */
    private static String PASSWORD = "ftptest";
    /**
     * FTP服务器基础目录
     */
    private static String BASEPATH = "/var/ftp/pub";
    /**
     * FTP客户端
     */
        ...
        try {
            ...
            if (!ftp.changeWorkingDirectory(BASEPATH+filePath)) {
                ...
                for (String dir : dirs) {
                    ...
                    tempPath += "/" + dir;
                    if (!ftp.changeWorkingDirectory(tempPath)) { if (!ftp.makeDirectory(tempPath)) {
                        return result;
                    } else {
    ...
    public static boolean downloadFile(String remotePath, String fileName, OutputStream outputStream) {
        ...
        try {
            ...
            for (FTPFile ff : fs) {
                if (ff.getName().equals(fileName)) {
                    ftp.enterLocalPassiveMode();
                    boolean b = ftp.retrieveFile(BASEPATH+remotePath + "/" + fileName, outputStream);
